...
from .site import Selectors, SiteConfig
...
def _fill(cls, data: dict[str, Any]):
    """Instancie une dataclass en ignorant les cles inconnues, mais bruyamment.

    Une cle inconnue est presque toujours une faute de frappe dans config.yaml
    qui ferait silencieusement tourner le bot avec la valeur par defaut.
    """
    known = {f.name for f in fields(cls)}
    unknown = set(data) - known
    if unknown:
        raise ValueError(f"cles inconnues dans la section {cls.__name__} : {sorted(unknown)}")
    return cls(**data)
...
def load(path: str | Path) -> AppConfig:
    ...
    selectors = _fill(Selectors, site_raw.pop("selectors", {}) or {})
    if "action_delay_s" in site_raw:
        site_raw["action_delay_s"] = tuple(site_raw["action_delay_s"])
    site = _fill(SiteConfig, {**site_raw, "selectors": selectors})
